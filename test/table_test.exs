defmodule ExAdminTest.TableTest do
  ...
  setup do
    table_options = %{fields: [:id, :name, :inserted_at], filter: "", sort_order: nil, page: %{page_number: 1}, path_prefix: "/admin/users?order=", scope: nil, selectable: true, selectable_column: true, sort: "desc"}
  ...
  describe "build_th" do
    ...
    test "link field", %{table_opts: table_options} do
      expected = "<th class='sortable th-id'><a href='/admin/users?sort_order=id_desc&page=1'>Id</a></th>"
      opts = {:id, %{link: true}}
      assert Table.build_th(opts, table_options) == expected
    end
    test "date field", %{table_opts: table_options} do
      expected = "<th class='sortable th-inserted_at'><a href='/admin/users?sort_order=inserted_at_desc&page=1'>Inserted At</a></th>"
      opts = {:inserted_at, %{}}
      assert Table.build_th(opts, table_options) == expected
    end
    test "sortable with scope", %{table_opts: table_options} do
      expected = "<th class='sortable th-id'><a href='/admin/users?sort_order=id_desc&page=1&scope=complete'>Id</a></th>"
      table_options = put_in table_options, [:scope], "complete"
      opts = {:id, %{}}
      assert Table.build_th(opts, table_options) == expected
    end
    test "filtered", %{table_opts: table_options} do
      expected = "<th class='sortable sorted-desc th-id'><a href='/admin/users?sort_order=id_asc&page=1&q%5Btotal_price_gt%5D=100'>Id</a></th>"
      table_options = %{table_options | filter: "&q%5Btotal_price_gt%5D=100", sort_order: {"id", "desc"}}
      opts = {:id, %{}}
      assert Table.build_th(opts, table_options) == expected
    end
    test "order asc", %{table_opts: table_options} do
      expected = "<th class='sortable sorted-asc th-id'><a href='/admin/users?sort_order=id_desc&page=1'>Id</a></th>"
      table_options = %{table_options | sort_order: {"id", "asc"}}
      opts = {:id, %{}}
      assert Table.build_th(opts, table_options) == expected
    end
